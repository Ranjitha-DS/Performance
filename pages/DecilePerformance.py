...
def write1():
    ...
    if(len(selected_options)==0):
        ...
        #df1 = df.head(3)
        st.dataframe(df.style
        .background_gradient(cmap='RdYlGn', subset=(df.index[0:9],['COLLECTION_EFF','RESOLUTION','AWS']))
        .set_properties(subset=['COLLECTION_EFF','RESOLUTION','AWS']))
    
     #----

        chart_df = df[['DECILE', '#FCE']]

        # chart_df = chart_df.astype(str)

        fig = go.Figure()
        fig.add_trace(go.Scatter(x=chart_df['DECILE'].astype(str), y=chart_df['#FCE'],
                    mode='lines+markers',
                    name='#FCE'))
        ...
        chart_df1 = df[['DECILE', 'FCE%','COLLECTION_EFF','RESOLUTION']]

        # chart_df = chart_df1.astype(str)

        fig1 = go.Figure()
        fig1.add_trace(go.Scatter(x=chart_df1['DECILE'].astype(str), y=chart_df1['FCE%'],
                    mode='lines+markers',
                    name='FCE%'))
        fig1.add_trace(go.Scatter(x=chart_df1['DECILE'].astype(str), y=chart_df1['COLLECTION_EFF'],
                    mode='lines+markers',
                    name='Collection Efficiency'))
        
        fig1.add_trace(go.Scatter(x=chart_df1['DECILE'].astype(str), y=chart_df1['RESOLUTION'],
                    mode='lines+markers',
                    name='Resolution'))


        fig1.update_layout(
           title="DECILE    Vs    FCE%, Collection Eff, Resolution",
           xaxis_title="DECILE",
           yaxis_title="Metrics Value",
           legend_title="Metrics",
            )
        ...
        chart_df2 = df[['DECILE', 'AWS']]

        # chart_df = chart_df1.astype(str)

        fig2 = go.Figure()
        fig2.add_trace(go.Scatter(x=chart_df2['DECILE'].astype(str), y=chart_df2['AWS'],
                    mode='lines+markers',
                    name='FCE%'))
